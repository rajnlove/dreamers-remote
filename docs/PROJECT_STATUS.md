...
Not deployed yet at the time of writing: the backend and upload images are
pinned by commit SHA (see the entry below and CONTAINERS.md), so this needs a
pin bump to reach production. The web image tracks `:latest` and updates on its
own. Order doesn't matter — an old UI ignores the new fields, and a new UI
against an old backend simply shows Legacy/Unknown.
