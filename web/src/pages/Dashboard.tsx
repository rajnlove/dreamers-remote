        ...
        <main className="studio-content" id="studio-content">
          ...
          <section id="workstations" className="studio-workstations" aria-label="Workstations">
            {machines.map((machine) => (
              <WorkstationCard key={machine.id} username={user.username} workstation={machine} status={statusById.get(machine.id)} stale={!!errors.status || !!errors.machines} onChanged={() => void refresh()} />
            ))}
            {machines.length === 0 && (
